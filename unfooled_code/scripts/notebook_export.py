...
# Cached Hugging Face images locally (no generation)
...
#Counter-Forensic Attacks (Red-Team Pool)
...
#Metrics roc_curve, f1_score, matthews_corrcoef, log_loss
...
#Qualitative: Predictions & Heatmaps
...
# Visuals: Overlays + all-attacks Grid
def overlay_heatmap(img_pil, mask01, cmap_name="turbo", alpha=0.45, alpha_pow=0.6, thr=0.02):
    m = np.asarray(mask01, dtype=np.float32)
    if m.ndim == 3: m = m.squeeze()
    m = np.clip(m, 0.0, 1.0)
